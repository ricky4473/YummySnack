...
def productslist(modelqueryset):
    def inner(item):
        ...
        'saleprice':saleprice,
        "id":item.id,
        "rating":math.ceil(reduce(lambda x,y:x+y.rating,models.OrderItemsComment.objects.filter(order_items__product__id=item.id),0)/(len(models.OrderItemsComment.objects.filter(order_items__product__id=item.id)) or 1))
...
def littleloopdictfn():
    ...
    for item in models.LittleLoopPicker.objects.all().order_by('-priority'):
        ...
        if item.product.all() :
            filterdict['id__in']=[item.id for item in item.product.all()]   #manytomany item.product is == models.Product.objects
        if not filterdict and not item.product.all():
            filterdict={"id":0}

        littleloopdict[item.title]=productslist(models.Product.objects.filter(**filterdict).order_by('sale_rank')),filterdict
...
def index(request):
    ...
    if request.method=='POST':
        ...
        data=models.Product.objects.filter(name__icontains=obj.get('keywords')).order_by("sale_rank")[0:10]
...
def products(request):
    ...
    productorderdict={"best-selling":("sale_rank","Best selling"),"title-ascending":("name","Alphabetically, A-Z"),"title-descending":("-name"," Alphabetically, Z-A"),"price-ascending":("price","Price, low to high"),"price-descending":("-price","Price, high to low"),"created-ascending":("id","Date, old to new"),"created-descending":("-id"," Date, new to old")}
    if request.method=="POST":
        cid=request.POST.get('cid',0)
        bid=request.POST.getlist('bid') #list
        pricerange=request.POST.get('pricerange', 0)
        productorder=productorderdict[request.POST.get("productorder","best-selling")][0]
        ...
        if request.POST.get('filterdictfortype'):
            productfilterdict={}
            productfilterdict={"name__icontains":request.POST.get('filterdictfortype')}
        page=request.POST.get('page',1)
        pagecode=Pagenation(request,models.Product.objects.filter(**productfilterdict).order_by(productorder),page_size=show)
        # print(pagecode.html())
        
        
        filteredpl=productslist(pagecode.page_queryset)   #.filter(brand__id__in=[1,2]    .filter(brand__id__in=previousbrand)    .filter(brand__id=bid)
    ...
    pagecode=Pagenation(request,models.Product.objects.all().order_by('sale_rank'),page_size=20)
    productclass=models.ProductClass.objects.all().order_by('name')
    ...
    context={"products":pl,'class':productclass,'brands':bl,"productorderdict":productorderdict,'pagecode':pagecode.html()}
    return render(request,'products/index.html',context)
...
def products_product(request,nid):
    ...
    productslength=len(list(productslist(models.Product.objects.filter(id=nid))))
    comments= commentslist(
        models.OrderItemsComment.objects.filter(order_items__product__id=nid).order_by('comment_day'))
    context={'products':products,'comments':comments,"productslength":productslength,**littleloopdict}
...
def accounts(request):
    ...
    if request.method=="POST":
        form=UserEditModelForm(request.POST,request.FILES,instance=request.user)
        
        if form.is_valid():
            
            forminstance=form.save(commit=False)
            if form.cleaned_data.get('password',0):
                forminstance.password=make_password(form.cleaned_data.get('newpassword',0))
            else:
...
def accounts_address(request):
    ...
    context={'addressshipping':addressshipping,"addressbilling":addressbilling}
...
def accounts_address_add(request,typenum):
    ...
    if request.method=="POST":
        if models.Address.objects.filter(user=request.user,type=typenum).exists():
            lastprioritynum=models.Address.objects.filter(user=request.user,type=typenum).order_by('-priority')[0].priority
        else:
            lastprioritynum=0
        ...
        if form.is_valid():
            ...
            form.instance.save()
            context={"form":form ,"typenum":typenum,"data_dict":{"status":1,"msg":"增加成功"}}
            return render(request,'accounts/address_add.html',context)
        context={"form":form ,"typenum":typenum,"data_dict":{"status":0,"msg":"驗證失敗"}}
        return render(request,'accounts/address_add.html',context)
...
def accounts_orders(request):
    proces_orders=request.user.order_set.filter(status__in=[1,2],cartcomplete=True)
...
def accounts_orderitemcommentadd(request,nid):
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            ...
            form.instance.save()
            context={"form":form ,"nid":nid,"data_dict":{"status":1,"msg":"新增評論成功"}}
            return render(request,'accounts/accounts_orderitemcommentadd.html',context)

        context={"form":form ,"nid":nid,"data_dict":{"status":0,"msg":"驗證失敗"}}
        return render(request,'accounts/accounts_orderitemcommentadd.html',context)
...
def accounts_orderitemcommentedit(request,nid):
    ...
    if request.method =="POST":
        ...
        if form.is_valid():
            ...
            context={"form":form ,"nid":order_items_id,"data_dict":{"status":1,"msg":"更改成功"}}
            return render(request,'accounts/accounts_orderitemcommentedit.html',context)
       
        context={"form":form ,"nid":order_items_id,"data_dict":{"status":0,"msg":"驗證失敗"}}
        return render(request,'accounts/accounts_orderitemcommentedit.html',context)
...
def accounts_orderitemcommentdelete(request,nid):
    # nid = .OrderItemsComment id 
    order_items_id=models.OrderItemsComment.objects.get(id=nid).order_items.id
    ...
    if request.method=="POST":
        ...
        if form.is_valid():
            if user:
                ...
                for id in cart:
                    ordereditem,created=models.OrderedItem.objects.get_or_create(order=order,product_id=id)
...
class UserModelForm(BootStrapModelForm):
    password=forms.CharField(widget=forms.PasswordInput(),validators=[RegexValidator('^.*(?=.{6,10})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*\d).*$','plz input 6-20 characters, at least one lowercase ,uppercase,digits for 0-9')])
...
class UserEditModelForm(BootStrapModelForm):
    password=forms.CharField(label='Please Type Old Password',widget=forms.PasswordInput(),required=False)
    newpassword=forms.CharField(label='New Password',widget=forms.PasswordInput(),validators=[RegexValidator('^.*(?=.{6,10})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*\d).*$','plz input 6-20 characters, at least one lowercase ,uppercase,digits for 0-9')],required=False)
    newpasswordconfirm=forms.CharField(label='Confirm New Password',widget=forms.PasswordInput(),required=False)
    class Meta:
        model=models.User
...
class AddressShippingForm(BootStrapModelForm):
    class Meta:
        model=models.Address
        exclude=["user",'priority','type',] #'first_name','last_name','phone'
...
AddressFromSet=modelformset_factory(models.Address,exclude=["user","priority","type"],extra=2,form=BootStrapModelForm,formset=MyFormSet)
    ...
    if request.method=="POST":
        ...
        if form.is_valid() and formset.is_valid():
            ...
            if models.User.objects.filter(email=email).exists():
                context={'form':form,"formset":formset,'data_dict':{'status':0,'errors_msg': "電郵已註冊"}}
                ...
                for item in UserModelForm().fields.keys():
                    ...
                    if item == "avatar":
                        userdatadict[item]=request.FILES.get(item,"avatars/defaulticon.jpeg")
                ...
                # return JsonResponse(data_dict)
                context={'form':form,"formset":formset,'data_dict':{"status":1,"msg":"註冊成功"}}
                return render(request,'accounts/register.html',context)
        # data_dict={'status':False,"msg":form.errors,"formsetmsg":formset.errors,'form':form,"formset":formset}
        # return HttpResponseBadRequest(json.dumps(data_dict))
        context={'form':form,"formset":formset,'data_dict':{'status':0,'msg':'驗證失敗，請重新輸入。'}}
        return render(request,'accounts/register.html',context)
    form=UserModelForm()
...
def accounts_reset(request):
    ...
    if 1:
        email=EmailMessage('halo','hahahhahahahahhaa',settings.EMAIL_HOST_USER,[settings.EMAIL_HOST_USER,'lee670z@gmail.com'])
...
def management(request):
    labeldatadict={"label":[],"data":[],'newuserorder':[],"olduserorder":[],'weeklytotalorder':0}
    ...
    orders=models.Order.objects.filter(cartcomplete=True).order_by('status','-paid_time')
...
def cart(request):
    if request.method=="POST":
        ...
        voucherfilter=models.Voucher.objects.filter(code=vouchercode,end_date__gte=datetime.datetime.now().date())
        if voucherfilter.exists():
            vouchercode=voucherfilter.first()
            if vouchercode.usage_limit >vouchercode.used:
                ordercart=request.user.order_set.filter(cartcomplete=False).first()
            ...
            vouchercode=models.Voucher.objects.filter(code=vouchercode).first()
        return JsonResponse({})
...
def checkout(request):
    ...
    if request.user.is_authenticated:
        row_object=models.User.objects.get(id=request.user.id).address_set.get(type=1,priority=1)
        form=AddressModelForm(instance=row_object)
        shippingdict=addressdict(models.Address.objects.filter(type=1,user=request.user).order_by('priority'))
        billingdict=addressdict(models.Address.objects.filter(type=2,user=request.user).order_by('priority'))
...
def processorder(request):
    ...
    #stripe part
        ...
        if order.voucher:
            ...
            percent_off['percent_off']=(100-percent_off.get('percent_off',0))*(100-discount_value)
            percent_off['percent_off']=round(100-percent_off['percent_off']/100)
